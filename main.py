...
# todo не работает прерыватель abort все равно идет попытка записи в базу данных
# abort from any
...
async def make_changes_command(message: types.Message):
    ...
    # todo обработать нажатие на кнопки
...
async def send_welcome(message: types.Message):
    ...
    await message.answer(f"Your id is {message.from_user.id} and your name is {message.from_user.username}")
